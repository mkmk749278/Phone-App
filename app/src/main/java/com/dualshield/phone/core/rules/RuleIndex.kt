 ...
 *
 * Exact matches are a hash lookup; everything else is a short pre-filtered list. The point
 * is that [match] does no allocation-heavy or I/O work, because it runs inside
 * `CallScreeningService.onScreenCall`, where Android gives us only a few seconds.
 */
class RuleIndex private constructor(
    private val exact: Map<String, CompiledRule>,
    ...
    /** Returns the first rule that matches [info], or null. Never throws. */
    fun match(info: PhoneNumberInfo): CompiledRule? {
        specials.firstOrNull { matchesSpecial(it.special, info) }?.let { return it }

        if (!info.hasDigits) return null

        for (candidate in info.matchCandidates) {
            exact[candidate]?.let { return it }
        }
        prefixes.firstOrNull { rule ->
            info.matchCandidates.any { it.startsWith(rule.pattern) }
        }?.let { return it }
        contains.firstOrNull { rule ->
            info.matchCandidates.any { it.contains(rule.pattern) }
        }?.let { return it }
    ...
    companion object {
        ...
        fun build(rules: List<CompiledRule>): RuleIndex {
            ...
            val exact = LinkedHashMap<String, CompiledRule>()
            ...
            for (rule in ordered) {
                when (rule.patternType) {
                    PatternType.EXACT -> exact.putIfAbsent(rule.pattern, rule)
